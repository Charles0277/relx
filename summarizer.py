...
def summarize_text(text):
    ...
    for i in range(0, len(tokens), max_chunk_length):
        ...
        # Summarize the chunk
        try:
            summary_chunk = summarizer(chunk_text, max_length=150, min_length=30, do_sample=False)
            summaries.append(summary_chunk[0]['summary_text'])
        except Exception as e:
            print(f"An error occurred during summarization of a chunk: {e}")
...
def main():
    parser = argparse.ArgumentParser(description="Summarize text from one or more files and extract named entities.")
    parser.add_argument("files", nargs='+', help="Paths to the text files to be summarized.")
